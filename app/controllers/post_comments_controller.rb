  ...
  def create
    if params[:post_comment][:content].empty?
      ...
      if @post_comment.save
        ...
        ActionCable.server.broadcast("post_comment_channel", { post_comment: @post_comment.html_template(@current_user), post_comment_count: @post_comment.html_template_count, micropost_id: @post_comment.micropost_id, method: "comment_create" })
        ActionCable.server.broadcast("flash_channel", { flash: flash, user_id: current_user.id })
        return
      else
